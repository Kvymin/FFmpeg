    ...
    if (in_fmt) {
        if (in_fmt->extensions) {
            ...
            const char *str = "ts,m2t,m2ts,mts,mpg,m4s,mpeg,mpegts"
                              ...
                              ",bmp,jpeg,jpg,png,gif"  // sites may disguise TS segments as images
                            ;
...
static int has_mpegts_sync_after(const uint8_t *buf, int buf_size, int offset)
{
    enum {
        TS_PACKET_SIZE = 188,
        TS_SYNC_BYTE = 0x47,
    ...
    /* Keep detection tied to the image boundary while allowing a short marker. */
    for (int start = offset;
         start - offset < TS_PACKET_SIZE &&
         buf_size - start >= 1 + (TS_PACKETS - 1) * TS_PACKET_SIZE;
         start++) {
        int i;

        for (i = 0; i < TS_PACKETS; i++) {
            int packet = start + i * TS_PACKET_SIZE;
...
static int probe_image_wrapped_mpegts(AVIOContext *pb, const char *fmt_name)
{
    ...
    if (!strcmp(fmt_name, "png_pipe")) {
        ...
        int iend_size = sizeof(png_iend);

        if (buf_size < sig_size || memcmp(buf, png_signature, sig_size))
            return 0;

        for (int pos = sig_size; pos + iend_size <= buf_size; pos++)
            if (!memcmp(buf + pos, png_iend, iend_size) &&
                has_mpegts_sync_after(buf, buf_size, pos + iend_size))
                return 1;
        ...
        for (int pos = 2; pos + 1 < buf_size; pos++)
            if (buf[pos] == 0xff && buf[pos + 1] == 0xd9 &&
                has_mpegts_sync_after(buf, buf_size, pos + 2))
                return 1;
        ...
        return has_mpegts_sync_after(buf, buf_size, file_size);
    } else if (!strcmp(fmt_name, "gif")) {
        if (buf_size < 13 ||
            (memcmp(buf, "GIF87a", 6) && memcmp(buf, "GIF89a", 6)))
            return 0;

        for (int pos = 13; pos < buf_size; pos++)
            if (buf[pos] == 0x3b &&
                has_mpegts_sync_after(buf, buf_size, pos + 1))
                return 1;
    }
...
fail:
    ...
    if (primary && primary->type == PLS_TYPE_EVENT && !primary->finished) {
        struct playlist *p = primary;
        int64_t duration = 0;
...
static int hls_read_header(AVFormatContext *s)
{
    ...
    if (primary) {
        ...
        /* Finished and EVENT playlists retain every segment from the start. */
        if (primary->finished || primary->type == PLS_TYPE_EVENT) {
            int64_t duration = 0;
...
static const AVOption hls_options[] = {
    ...
    {"allowed_segment_extensions", "List of file extensions that hls is allowed to access",
        ...
        {.str = "3gp,aac,avi,ac3,bmp,eac3,flac,gif,jpeg,jpg,mkv,m3u8,m4a,m4s,m4v,mpg,mov,mp2,mp3,mp4,mpeg,mpegts,ogg,ogv,oga,png,ts,vob,vtt,wav,webvtt"
            ",cmfv,cmfa" // Ticket11526 www.nicovideo.jp
            ",ec3"       // part of Ticket11435 (Elisa Viihde (Finnish online recording service))
            ",fmp4"      // https://github.com/yt-dlp/yt-dlp/issues/12700
            ",html"      // https://flash1.bogulus.cfd/
